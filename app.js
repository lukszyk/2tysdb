...
const FIREBASE_COLLECTION_RECENT = 'tysiac_recent_games';
    ...
    unsubscribeLiveSync = savedGameStateRef.onSnapshot((doc) => {
        ...
        if (!doc.exists) {
            if (gameState.isActive) {
                resetGame();
                showTemporaryMessage('Gra została usunięta lub zakończona na innym urządzeniu.', false);
            }
            return;
        }
...
async function handleShowLoadScreen() { 
    ...
    try { 
        ...
        if (doc.exists) { 
            ...
            loadGameMessage.textContent = `Znaleziono zapisaną grę w chmurze (${playersNames}). Czy chcesz ją wczytać i śledzić wynik na żywo?`; 
...
async function saveCurrentGameState(showNotification = true) { 
    ...
    try { 
        ...
        gameState.loadedFromFirebase = true; 
        if (showNotification) showTemporaryMessage('Stan gry został zapisany w chmurze!', false); 
    } catch (error) { 
        console.error("Błąd zapisu stanu gry:", error); 
        if (showNotification) showTemporaryMessage('Błąd podczas zapisu stanu gry!', true); 
    } 
}
...
async function deleteSavedGameState() { 
    ...
    if (!db || !savedGameStateRef) return; 
    try { 
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        grid: {
                            color: (context) => {
                                if (context.tick.value === 1000) return 'rgba(52, 211, 153, 0.45)';
                                if (context.tick.value === 800) return 'rgba(251, 146, 60, 0.5)';
                                if (context.tick.value === 0) return 'rgba(248, 113, 113, 0.35)';
                                return '#1c332c';
                            },
                            lineWidth: (context) => {
...
function renderRoundHistory() { 
    if (gameState.history.length === 0) { 
        roundHistoryContainer.innerHTML = '<p class="text-emerald-500/60 text-xs text-center p-2">Brak dodanych rund.</p>'; 
        return; 
    } 
...
function updateScoreValues() { 
    ...
    gameState.players.forEach((p, i) => { 
        ...
        if (scoreEl) { 
            ...
            scoreEl.classList.toggle('text-emerald-400', p.score < 800); 
        } 
...
function generatePlayerNameInputs() { 
    ...
    for (let i = 1; i <= count; i++) { 
        playerNamesContainer.innerHTML += `<div><label for="player${i}" class="block mb-1 text-xs font-semibold text-emerald-200/70">Imię gracza ${i}:</label><input type="text" id="player${i}" class="input-field font-medium" placeholder="Gracz ${i}" value="${defaultPlayerNames[i - 1] || `Gracz ${i}`}"></div>`; 
    } 
}
...
function handleBackToMenu() { 
    if (gameState.isActive) { 
        const message = gameState.loadedFromFirebase 
            ? 'Wyjść do menu głównego? Zapis gry pozostanie w chmurze i będzie można do niego wrócić.' 
            : 'Porzucić obecną grę? Postęp lokalny zostanie utracony.'; 
...
function showCustomConfirm(msg, onConfirm, showNo = true) { 
    ...
    if (!showNo) { 
        ...
        confirmYesBtn.classList.remove('bg-rose-600', 'w-1/2'); 
        confirmYesBtn.classList.add('btn-primary', 'w-full'); 
    } else { 
        confirmYesBtn.textContent = 'Tak'; 
        confirmYesBtn.classList.add('bg-rose-600', 'w-1/2'); 
...
async function saveStats(winner) { 
    ...
    try { 
        ...
        await db.collection(FIREBASE_COLLECTION_RECENT).add({
...
async function showStats() { 
    ...
    if (sortedWins.length > 0) { 
        ...
        sortedWins.forEach(([name, wins]) => { 
            ...
            summaryHtml += `<div class="flex justify-between items-center border-b border-emerald-900/40 pb-1"><span>${name}:</span> <span class="font-bold"><span class="text-emerald-400">${wins}</span> wygrane (${winPercentage}%)</span></div>`; 
        }); 
        summaryHtml += `</div>`; 
    } else { 
